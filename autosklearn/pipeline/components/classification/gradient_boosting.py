...
class GradientBoostingClassifier(
    ...
    def get_hyperparameter_search_space(dataset_properties=None):
        ...
        my_name = 'HistGradientBoostingClassifier_'
        ...
        max_leaf_nodes = Config.get_value(my_name, UniformIntegerHyperparameter(
            name="max_leaf_nodes", lower=3, upper=2047, default_value=31, log=True))
        max_bins = Constant("max_bins", 255)
        l2_regularization = Config.get_value(my_name, UniformFloatHyperparameter(
            name="l2_regularization", lower=1E-10, upper=1, default_value=1E-10, log=True))

        early_stop = Config.get_value(my_name, CategoricalHyperparameter(
        ...
        n_iter_no_change = Config.get_value(my_name, UniformIntegerHyperparameter(
            name="n_iter_no_change", lower=1, upper=20, default_value=10))
        validation_fraction = Config.get_value(my_name, UniformFloatHyperparameter(
        ...
        if Config.check_value("valid", early_stop) and Config.check_value("train", early_stop):
            n_iter_no_change_cond = InCondition(
                n_iter_no_change, early_stop, ["valid", "train"])
            cs.add_condition(n_iter_no_change_cond)

        if Config.check_value("valid", early_stop):
            validation_fraction_cond = EqualsCondition(
                validation_fraction, early_stop, "valid")
            cs.add_condition(validation_fraction_cond)
